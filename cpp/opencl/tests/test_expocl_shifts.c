...
int main(int argc, char** argv) {
  ...
  double* pout = (double*) malloc(N*sizeof(double));
  transEvent(p0,pout,expo);
  memcpy(p0,pout,N*sizeof(double));

  int wrong = 0;
  for (i = 0; i < N; ++i) if (fabs(b[i]-p0[i]) > 1e-8) wrong++;

  expocl_samp_b(ecl);
  expocl_copy_bx(ecl);
  expocl_read_b(ecl,b);

  sampleEvent(p0,pout,expo);
  memcpy(p0,pout,N*sizeof(double));
