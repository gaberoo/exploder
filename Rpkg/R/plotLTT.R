.get.lineages <- function(tree,nroot=0) {
  tt <- rev(tree[,2])
  sevents <- tt %in% c(0,2,4)
  bevents <- tt %in% c(1)
  tt[sevents] <- 0
  tt[!(sevents | bevents)] <- 0.5
  rev(nroot+cumsum(2*tt-1))
}

plotLTT <- function(trees,nroot=1,col=rgb(.5,.5,.5,.5),xlab="Time",
                    ylab="Lineages",log="y",add=FALSE,
                    plot=TRUE,plot.tree=NA,type="s",
                    axes=plot,...) 
{
  max.time <- sapply(trees,function(t) max(t[,1]))
  lineages <- lapply(trees,.get.lineages,nroot=nroot)
  ...
  if (! plot) {
    ...
    if (! add) {
      if (! is.na(plot.tree) & length(trees) == 1) {
        ...
        lines(rev(max.time-trees[[1]][,1]),rev(ll),type=type,lwd=5,col="white")
        lines(rev(max.time-trees[[1]][,1]),rev(ll),type=type,lwd=3,col="black")
